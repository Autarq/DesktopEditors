...
<#
.SYNOPSIS
    Local Windows build for AUTARQ Office.
...
try {
    ...
    if ($BuildCommon) {
        ...
        # The AUTARQ brand does not require external branding credentials.
...
"@
    ...
    Write-Step "9a. Removing bundled AI Agent plugins"
    foreach ($pluginId in @(
        '{9DC93CDB-B576-4F0C-B55E-FCC9C48DD777}',
        '{F2402876-659F-47FB-A646-67B49F2B5AAA}'
    )) {
        $pluginPath = Join-Path $InstallDir "editors\sdkjs-plugins\$pluginId"
        if (Test-Path $pluginPath) {
            Remove-Item -Recurse -Force -LiteralPath $pluginPath
        }
    }
